document.addEventListener("DOMContentLoaded", async () => {
  ...
  async function loadScans() {
    ctList.innerHTML = '<div class="loader-container"><div class="loader"></div></div>';
    try {
      const data = await API.request("/api/visits/ct-scans");
      renderScans(data.visits);
    } catch (err) {
      ctList.innerHTML = `<p class="message error">Failed to load scans: ${err.message}</p>`;
    }
  }
  ...
  function renderScans(scans) {
    ...
    if (!scans.length) {
      ctList.innerHTML = '<div class="empty-state" style="text-align:center;color:var(--muted);padding:2rem;">No CT scan patients found for the last 48 hours.</div>';
      return;
    }

    ctList.innerHTML = scans.map((s, i) => `
    ...
    ctList.querySelectorAll("button[data-id]").forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        try {
          ...
          showMessage("ctMessage", "Failed to update: " + err.message, true);
          btn.disabled = false;
