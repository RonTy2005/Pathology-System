document.addEventListener("DOMContentLoaded", async () => {
  const user = getUser();
  if (!user || (user.role !== "usg_technician" && !isAdministrativeRole(user.role))) {
  ...
  const usgList = document.getElementById("usgList");
  ...
  async function loadScans() {
    usgList.innerHTML = '<div class="loader-container"><div class="loader"></div></div>';
    try {
      const data = await API.request("/api/visits/usg-scans");
      renderScans(data.visits);
    } catch (err) {
      usgList.innerHTML = `<p class="message error">Failed to load scans: ${err.message}</p>`;
    }
  }
  ...
  function renderScans(scans) {
    ...
    if (!scans.length) {
      usgList.innerHTML = '<div class="empty-state" style="text-align:center;color:var(--muted);padding:2rem;">No USG scan patients found for the last 48 hours.</div>';
      return;
    }

    usgList.innerHTML = scans.map((s, i) => `
    ...
    usgList.querySelectorAll("button[data-id]").forEach(btn => {
    ...
    bindImagingReportControls(usgList, loadScans);
  }
