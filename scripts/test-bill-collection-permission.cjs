...
test("older dues remain searchable and a collection-only user can settle them safely", async () => {
  ...
  try {
    ...
    await run("CREATE TABLE visits (id INTEGER PRIMARY KEY, patient_id INTEGER, bill_no TEXT, total REAL, amount_paid REAL, amount_due REAL, payment_mode TEXT, payment_status TEXT, status TEXT, created_at TEXT, associate_label TEXT, associate_id INTEGER, doctor_id INTEGER)");
    ...
    await run("INSERT INTO visits (id, patient_id, bill_no, total, amount_paid, amount_due, payment_status, status, created_at) VALUES (1, 1, 'OLD-DUE', 600, 2, 598, 'partial', 'registered', '2026-01-01')");
    ...
    assert.match(excessive.message, /exceeds the outstanding amount/);
